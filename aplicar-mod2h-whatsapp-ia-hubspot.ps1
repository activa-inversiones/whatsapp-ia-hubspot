...
# 0) BACKUP
# -------------------------------------------------------------------
$backupRoot = Join-Path $repo ("backups\mod2h_" + (Get-Date -Format "yyyyMMdd_HHmmss"))
...
# -------------------------------------------------------------------
# 5) Reemplazar waSendH / waSendMultiH completos
# Desde async function waSendH hasta antes de async function waRead
# -------------------------------------------------------------------
...
# -------------------------------------------------------------------
# 6) Reemplazar health
# Desde app.get("/health" hasta antes de app.get("/webhook"
# -------------------------------------------------------------------
...
# -------------------------------------------------------------------
# 7) Insertar endpoint /internal/operator-send antes del webhook POST
# -------------------------------------------------------------------
...
# -------------------------------------------------------------------
# 8) Insertar tracking inbound + takeover antes del reset
# -------------------------------------------------------------------
$idxReset = Find-LineIndex $lines '^\s*if \(/\^reset\|nueva cotizaci'
...
# 9) Ajustar reset
# -------------------------------------------------------------------
$idxResetSend = Find-LineIndex $lines '^\s*await waSendH\(waId, "🔄 Listo, empecemos de cero'
if ($idxResetSend -lt 0) { throw "No encontré la línea waSendH de reset" }
...
$lines = Replace-Range $lines $idxResetSend ($idxResetSend + 1) $resetReplacement
Info "Reset actualizado"

# -------------------------------------------------------------------
# 10) Agregar pushLeadEvent al final de zhUpsert
# -------------------------------------------------------------------
...
Info "pushLeadEvent agregado a zhUpsert"

# -------------------------------------------------------------------
# 11) Insertar trackQuoteEvent después de .catch(() => {});
# del bloque formal
# -------------------------------------------------------------------
$idxQuoteStart = Find-LineIndex $lines '^\s*zhUpsert\(ses, waId\)\.then\(\(\) => \{$'
if ($idxQuoteStart -lt 0) { throw "No encontré inicio del bloque de quote formal" }

$idxQuoteCatch = Find-LineIndex $lines '^\s*\}\)\.catch\(\(\) => \{\}\);$' $idxQuoteStart
if ($idxQuoteCatch -lt 0) { throw "No encontré .catch(() => {}); del bloque de quote formal" }
...
# -------------------------------------------------------------------
# 12) .env.example (solo documentación del repo)
# -------------------------------------------------------------------
...
Ok "M2H aplicado correctamente."
Write-Host ""
Write-Host "Siguiente paso:"
Write-Host "  git add ."
Write-Host '  git commit -m "mod2h integrate sales os bridge and operator handoff"'
Write-Host "  git push"