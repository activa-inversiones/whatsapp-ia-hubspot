...
function Insert-AfterExactNormalized {
    param(
        [string]$Content,
        [string]$Anchor,
        [string]$Insert,
    ...
    $idx = $c.IndexOf($a)
    if ($idx -lt 0) {
        throw "No encontré ancla exacta normalizada para: $Label"
    }
...
function Insert-BeforeExactNormalized {
    param(
        [string]$Content,
        [string]$Anchor,
        [string]$Insert,
...
# 0) BACKUP
# -------------------------------------------------------------------
$backupRoot = Join-Path $repo ("backups\mod2e_" + (Get-Date -Format "yyyyMMdd_HHmmss"))
...
# -------------------------------------------------------------------
# 1) RESTORE LIMPIO DESDE GIT HEAD
# -------------------------------------------------------------------
Push-Location $repo
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        ...
        if ($LASTEXITCODE -ne 0) {
            throw "No se pudo restaurar desde Git HEAD"
        }
    }
...
$index = Insert-AfterExactNormalized -Content $index -Anchor $companyAnchor -Insert $agentBlock -Label "AGENT_NAME + INTERNAL_OPERATOR_TOKEN"
...
$index = Insert-AfterExactNormalized -Content $index -Anchor $safeJsonAnchor -Insert $helpersBlock -Label "Helpers bridge"
...
$index = Insert-BeforeExactNormalized -Content $index -Anchor $webhookAnchor -Insert $operatorEndpoint -Label "/internal/operator-send"
...
$index = Insert-AfterExactNormalized -Content $index -Anchor $pdfAnchor -Insert $inboundBlock -Label "Tracking inbound + takeover"
...
if ($envNorm -notmatch '(?m)^AGENT_NAME=') {
    Info "Agregando bloque bridge a .env.example"
...
Ok "M2E aplicado correctamente."
Write-Host ""
Write-Host "Siguiente paso:"
Write-Host "  git add ."
Write-Host '  git commit -m "mod2e integrate sales os bridge and operator handoff"'
Write-Host "  git push"