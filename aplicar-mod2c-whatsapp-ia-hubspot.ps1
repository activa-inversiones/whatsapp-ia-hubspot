...
function Replace-Exact {
    ...
    if (-not $Content.Contains($Old)) {
        throw "No encontré el bloque exacto para: $Label"
    }
...
function Insert-AfterExact {
    ...
    if ($idx -lt 0) {
        throw "No encontré ancla exacta para: $Label"
    }
...
$backupRoot = Join-Path $repo ("backups\mod2c_" + (Get-Date -Format "yyyyMMdd_HHmmss"))
...
$index = Replace-Exact -Content $index -Old $oldImport -New $newImport -Label "Import salesOsBridge"

# ------------------------------------------------------------
# 2) AGENT_NAME + INTERNAL_OPERATOR_TOKEN
# ------------------------------------------------------------
$companyAnchor = @'
const COMPANY = {
...
if ($index -notmatch 'const\s+AGENT_NAME\s*=') {
    $index = Insert-AfterExact -Content $index -Anchor $companyAnchor -Insert $agentBlock -Label "AGENT_NAME + INTERNAL_OPERATOR_TOKEN"
} else {
    Warn "AGENT_NAME ya existe."
}

# ------------------------------------------------------------
# 3) Helpers bridge después de safeJson
# ------------------------------------------------------------
$safeJsonAnchor = @'
function safeJson(x) {
...
if ($index -notmatch 'function\s+fireAndForget\s*\(') {
    $index = Insert-AfterExact -Content $index -Anchor $safeJsonAnchor -Insert $helpersBlock -Label "Helpers bridge"
} else {
    Warn "Helpers bridge ya existen."
}

# ------------------------------------------------------------
# 4) waSendH / waSendMultiH
# ------------------------------------------------------------
$oldSendBlock = @'
...
$index = Replace-Exact -Content $index -Old $oldSendBlock -New $newSendBlock -Label "waSendH + waSendMultiH"
...
$index = Replace-Exact -Content $index -Old $oldHealth -New $newHealth -Label "/health"

# ------------------------------------------------------------
# 6) Endpoint interno operador antes de /webhook
# ------------------------------------------------------------
...
if ($index -notmatch 'app\.post\("/internal/operator-send"') {
    $index = Insert-BeforeExact -Content $index -Anchor $webhookAnchor -Insert $operatorEndpoint -Label "/internal/operator-send"
} else {
    Warn "/internal/operator-send ya existe."
}

# ------------------------------------------------------------
# 7) Tracking inbound + takeover humano
# ------------------------------------------------------------
$pdfAnchor = @'
...
if ($index -notmatch 'trackConversationEvent\.inbound') {
    $index = Insert-AfterExact -Content $index -Anchor $pdfAnchor -Insert $inboundBlock -Label "Tracking inbound + takeover"
} else {
    Warn "Tracking inbound + takeover ya existe."
}

# ------------------------------------------------------------
# 8) Reset con customer_name
# ------------------------------------------------------------
$oldReset = @'
...
$index = Replace-Exact -Content $index -Old $oldReset -New $newReset -Label "Reset waSendH"

# ------------------------------------------------------------
# 9) pushLeadEvent en zhUpsert
# ------------------------------------------------------------
$oldZhTail = @'
  if (ex?.id) {
...
$index = Replace-Exact -Content $index -Old $oldZhTail -New $newZhTail -Label "pushLeadEvent en zhUpsert"

# ------------------------------------------------------------
# 10) pushQuoteEvent formal
# ------------------------------------------------------------
$oldQuoteBlock = @'
...
$index = Replace-Exact -Content $index -Old $oldQuoteBlock -New $newQuoteBlock -Label "pushQuoteEvent formal"
...
Ok "M2C aplicado correctamente."
Write-Host ""
Write-Host "Siguiente paso:"
Write-Host "  git add ."
Write-Host '  git commit -m "mod2c integrate sales os bridge and operator handoff"'
Write-Host "  git push"