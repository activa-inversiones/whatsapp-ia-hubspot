...
# 0) BACKUP
# ------------------------------------------------------------
$backupRoot = Join-Path $repo ("backups\mod2real_" + (Get-Date -Format "yyyyMMdd_HHmmss"))
...
# Inserción antes de const STAGES = {
# ------------------------------------------------------------
$agentBlock = @'
...
$index = Insert-BeforeRegexNormalized `
    -Content $index `
    -Pattern 'app\.post\("/webhook",\s*async\s*\(req,\s*res\)\s*=>\s*\{' `
    -Insert $operatorEndpoint `
    -Label "/internal/operator-send"

# ------------------------------------------------------------
# 8) Tracking inbound + takeover
# Inserción después de la asignación userText del bloque PDF
# ------------------------------------------------------------
$oldPdfBlock = @'
...
$index = Replace-ExactNormalized -Content $index -Old $oldPdfBlock -New $newPdfBlock -Label "Tracking inbound + takeover"
...
# ------------------------------------------------------------
# 12) .env.example
# No toca Railway; solo documenta el repo
# ------------------------------------------------------------
$envBlock = @'
...
Ok "M2 real aplicado correctamente."
Write-Host ""
Write-Host "Siguiente paso:"
Write-Host "  git add ."
Write-Host '  git commit -m "mod2 real integrate sales os bridge and operator handoff"'
Write-Host "  git push"